public class WrenRDP extends RDP implements WrenTokens {
    ...
    private void program() {
        if(currTok == PROG_TOK){
            match(PROG_TOK); match(VARIABLE_TOK); match(IS_TOKEN); block();
        }
        else error();
    }
    private void block() {
    ...
    private void dec() {
        if(currTok == VAR_TOK)
        {
            match(VAR_TOK); varlist(); match(COL_TOK); type(); match(SEMICOLON_TOK);
        }
        else error();
    }
    private void type() {
    ...
    private void command() {
        ...
        {
            match(WHILE_TOK); boolexpr(); match(DO_TOK); commandseq(); 
            match(END_TOK); match(WHILE_TOK);
        }
    ...
    private void assign2() {
        ...
        else if(currTok == ASSIGN_TOK)
        {
            match(ASSIGN_TOK); intexpr();
        }
    }
    private void if2() {
        if(currTok == END_TOK)
    ...
    private void boolexpr() {
        if(currTok == TRUE_TOK || currTok == FALSE_TOK || currTok == NOT_TOK || currTok == LBRACK_TOK || currTok == INTCONST_TOK
                || currTok == LPAR_TOK || currTok == MINUS_TOK)
        {
            boolterm();
            boolexpr2();
    ...
    private void boolterm2() {
        if(currTok == AND_TOK)
        {
            match(AND_TOK); boolelement(); boolterm2();
        }
        else lambda();
    }
